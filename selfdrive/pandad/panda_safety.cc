...
void PandaSafety::setSafetyMode(const std::string &params_string) {
  ...
  std::string cp_sp = params_.get("CarParamsSP");
  if (!cp_sp.empty()) {
    AlignedBuffer aligned_buf_sp;
    capnp::FlatArrayMessageReader cmsg_sp(aligned_buf_sp.align(cp_sp.data(), cp_sp.size()));
    safety_param_sp = cmsg_sp.getRoot<cereal::CarParamsSP>().getSafetyParam();
  }
