...
Panda *connect(std::string serial="", uint32_t index=0) {
  ...
  // BluePilot: classic CAN on C3 (F4); CAN-FD only on C3X (TICI_TRES)
  const bool can_fd = getenv("TICI_TRES") != nullptr;
  for (int i = 0; i < PANDA_CAN_CNT; i++) {
    panda->set_can_fd_auto(i, can_fd);
  }
  // End BluePilot
...
void pandad_run(std::vector<Panda *> &pandas) {
  ...
  SubMaster sm({"carParams", "selfdriveState", "selfdriveStateSP"});
  PubMaster pm({"can", "pandaStates", "peripheralState"});
  PandaSafety panda_safety(pandas);
  Panda *peripheral_panda = pandas[0];
  bool engaged = false;
  bool engaged_mads = true;
