...
# BluePilot: load libusb from pip package on AGNOS (C3 USB compatibility)
try:
...
class Panda:
  ...
  # BluePilot: F4 devices for legacy bootstub MCU detection on C3
  F4_DEVICES = [b'\x01', b'\x03', b'\x06']  # WHITE, BLACK, DOS
  # End BluePilot
  ...
  def connect(self, claim=True, wait=False):
    ...
    # BluePilot: legacy bootstub hw_type detection for early C3 internal pandas
    # A) oldest bootstubs: no MCU/panda type info
    # B) newer bootstubs: panda type in USB bcdDevice
    # C) latest bootstubs: hw_type endpoint (0xc1)
    ...
    # BluePilot: select MCU type for firmware paths
      ...
      for device in context.getDeviceList(skip_on_error=True):
        if device.getVendorID() in cls.USB_VIDS and device.getProductID() in cls.USB_PIDS:
          ...
          if serial is None or this_serial == serial:
            ...
            # BluePilot: read hw_type from bcdDevice for legacy bootstubs
  ...
  def get_type(self):
    ...
    # BluePilot: old bootstubs don't implement hw_type endpoint, use bcdDevice fallback
  ...
  # BluePilot: MCU type detection for C3 legacy bootstubs
