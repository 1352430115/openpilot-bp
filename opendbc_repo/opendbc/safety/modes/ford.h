...
// BluePilot: bp-4.0 panda safety — only curvature + path_angle checks (no path_offset/curvature_rate gates)
// End BluePilot
...
static bool ford_tx_hook(const CANPacket_t *msg) {
  ...
  // BluePilot: bp-4.0 panda lateral safety — curvature + path_angle ROC only; ramp_type 3 = hands-on override
  // End BluePilot
  if (msg->addr == FORD_LateralMotionControl) {
    ...
    int path_angle = (int)raw_path_angle - (int)FORD_INACTIVE_PATH_ANGLE;
  ...
  if (msg->addr == FORD_LateralMotionControl2) {
    ...
    unsigned int raw_ramp_type = (msg->data[0] >> 1) & 0x3U;
